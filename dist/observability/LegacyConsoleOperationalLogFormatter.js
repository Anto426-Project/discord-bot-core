...
const compareEntries = (left, right) => left[0].localeCompare(right[0], "en");
const formatTime = (occurredAt) => {
    if (!Number.isFinite(occurredAt.getTime())) {
        return "invalid-time";
    }
    return occurredAt.toISOString().slice(11, 23);
};
const resolveVisualLevel = (record) => {
...
const recordDetails = (record) => {
    ...
    if (references.length > 0) {
        details.push(`References: ${references
            .map(([key, value]) => `${cleanText(key, 60)}=${cleanText(value, DETAIL_LIMIT)}`)
            .join(" · ")}`);
    }
    if ("attributes" in record) {
        ...
        if (visibleAttributes.length > 0) {
            ...
            details.push(`Details: ${visibleAttributes
                .map(([key, value]) => `${cleanText(key, 60)}=${formatPrimitive(key, value)}`)
    ...
    if (record.kind === "error" ||
        record.kind === "collector_error" ||
        record.kind === "persistent_error") {
        const fingerprint = record.kind === "error" || record.kind === "persistent_error"
            ? ` · fingerprint=${record.fingerprintSha256.slice(0, 12)}`
            : "";
        details.push(`Error: ${cleanText(record.errorClass, DETAIL_LIMIT)} · code=${cleanText(record.errorCode, DETAIL_LIMIT)} · retryable=${record.retryable ? "yes" : "no"}${record.providerCode === undefined
            ? ""
            : ` · provider=${cleanText(record.providerCode, DETAIL_LIMIT)}`}${record.attemptNo === undefined ? "" : ` · attempt=${record.attemptNo}`}${fingerprint}`);
...
const compactRecordDetails = (record) => {
    const details = [];
    ...
    if (references.length > 0) {
        details.push(references
            .map(([key, value]) => `${cleanText(key, 40)}=${cleanText(value, 80)}`)
            .join(" "));
    }
    if (isError) {
        details.push(`${cleanText(record.errorClass, 60)}/${cleanText(record.errorCode, 100)}${record.retryable ? " retryable" : ""}`);
        if (record.redactedStack !== undefined) {
            const firstLine = record.redactedStack.split(/\r?\n/, 1)[0];
            if (firstLine !== undefined) {
                const cause = cleanText(firstLine, 180).replace(new RegExp(`^${record.errorClass.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}:\\s*`), "");
                if (cause !== "-") {
        ...
        if (attributes.length > 0) {
            details.push(attributes
                .map(([key, value]) => `${cleanText(key, 40)}=${formatPrimitive(key, value)}`)
                .join(" "));
        }
    }
...
export class LegacyConsoleOperationalLogFormatter {
    ...
    constructor(options = {}) {
        ...
        this.columns = Number.isFinite(requestedColumns)
            ? Math.min(MAXIMUM_WIDTH, Math.max(MINIMUM_WIDTH, Math.floor(requestedColumns)))
            : DEFAULT_WIDTH;
    }
    format(record) {
    ...
    formatSinkFailure(failure, sourceRecord, occurredAt) {
        ...
        const summary = `Operational log sink failed: ${safeFailure}; sink=${safeSink}; source=${cleanText(sourceRecord.eventKey, DETAIL_LIMIT)}`;
        const prefix = `[${formatTime(occurredAt)}] [❌ ERROR] [operational-logging]`;
        return `${this.paint(prefix, 31)} ${summary}`;
    }
    renderCompact(record, level) {
        const prefix = `[${formatTime(record.occurredAt)}] [${level.symbol} ${level.label}] [${cleanText(record.component, 80)}]`;
        const main = `${cleanText(record.eventKey, 100)} — ${cleanText(record.summary, SUMMARY_LIMIT)}`;
        const details = compactRecordDetails(record);
        return `${this.paint(prefix, level.ansi)} ${main} | ${details}`;
    }
    renderExpanded(record, level) {
        const innerWidth = this.columns - 4;
        const border = (text) => this.paint(text, level.ansi);
        const line = "─".repeat(innerWidth + 2);
        const renderLine = (value, color = false) => {
        ...
        const renderWrapped = (value) => wrapText(value, innerWidth).map((lineValue) => renderLine(lineValue));
        const header = `${level.symbol} ${level.label}  ${cleanText(record.component, innerWidth - 14)}`;
        const event = `[${formatTime(record.occurredAt)}] ${cleanText(record.eventKey, innerWidth - 16)}`;
        const details = recordDetails(record).flatMap((detail) => renderWrapped(`• ${detail}`));
        return [
            border(`╭${line}╮`),
...
//# sourceMappingURL=LegacyConsoleOperationalLogFormatter.js.map