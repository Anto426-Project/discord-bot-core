 ...
 * supported.
 */
export const LOCALIZATION_LOCALES = Object.freeze([
    ...SUPPORTED_LOCALES
]);
 ...
 */
export const normalizeLocalizationLocale = (value) => {
    if (typeof value !== "string") {
        throw new LocalizationError("LOCALIZATION.UNSUPPORTED_LOCALE", 'The locale must resolve explicitly to "it" or "en".', { value });
    }
    const normalized = value.trim().replaceAll("_", "-").toLowerCase();
    if (!LOCALE_PATTERN.test(normalized)) {
        throw new LocalizationError("LOCALIZATION.UNSUPPORTED_LOCALE", `Unsupported locale "${value}". The only supported languages are "it" and "en".`, { value });
    }
...
//# sourceMappingURL=LocalizationLocale.js.map