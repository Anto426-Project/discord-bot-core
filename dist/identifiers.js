...
export const parseDiscordSnowflake = (value, label = "Discord identifier") => {
    const normalized = value.trim();
    if (!SNOWFLAKE_PATTERN.test(normalized)) {
        throw new DiscordCoreError("DISCORD_INVALID_INPUT", `${label} is not a valid Discord snowflake.`, false);
    }
    return normalized;
};
export const parseStableBotKey = (value, label = "key") => {
    const normalized = value.trim();
    if (!STABLE_KEY_PATTERN.test(normalized)) {
        throw new DiscordCoreError("DISCORD_INVALID_INPUT", `${label} must be a stable lowercase identifier.`, false);
    }
    return normalized;
};
export const discordInteractionCorrelationId = (interactionId) => {
...
export const deterministicDiscordNonce = (deliveryId, destinationId) => {
